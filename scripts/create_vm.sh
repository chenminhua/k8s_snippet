...
az network vnet subnet create -g test --vnet-name testvnet -n testsubnet --address-prefix 10.0.0.1/24
...
az vm create -g test -n tv1 --image CentOS --nics test1-nic --admin-username azure --ssh-key-value ~/.ssh/ata.pub
